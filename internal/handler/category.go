...
func (h *Handler) deleteCategory(c *gin.Context) {
	...
	err := h.CategoryService.Delete(c.Request.Context(), input.Name)
	if err != nil {
		newErrorResponse(c, http.StatusInternalServerError, err.Error())
		log.Error("error added category", err.Error())
		return
...
func (h *Handler) editCategory(c *gin.Context) {
	...
	id, err := h.CategoryService.Edit(c.Request.Context(), input.Name)
	if err != nil {
		newErrorResponse(c, http.StatusInternalServerError, err.Error())
		log.Error("error added category", err.Error())
		return
...
func (h *Handler) getAllCategory(c *gin.Context) {
	...
	categoryies, err := h.CategoryService.GetAllProducts(c.Request.Context(), input.Tag)
	if err != nil {
		newErrorResponse(c, http.StatusInternalServerError, err.Error())
